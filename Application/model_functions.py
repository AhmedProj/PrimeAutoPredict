...
transformation_test = transforms.Compose(
    [transforms.Resize(size=(240, 180), antialias=True), transforms.ToTensor()]
)
...
def transform_fly(samples):
    samples["pixel_values"] = [
        transformation_train(image) for image in samples["image"]
    ]
    del samples["image"]
    return samples
...
def predict(img):
    ...
    return probs_test  # np.argmax(probs_test).item()
